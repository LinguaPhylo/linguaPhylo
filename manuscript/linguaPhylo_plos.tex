...
The most general phylogenetic tools are Bayesian methods include BEAST \cite{beast}, BEAST 2 \cite{bouckaert2014beastanalysis,beast2}, MrBayes \cite{mrbayes} and
RevBayes \cite{revbayes} 
...
Previous attempts to address model specification of Bayesian phylogenetic analyses include BEAST-style XMLs (eXtensible Markup Language) developed for the BEAST software \cite{beast,bouckaert2014beastanalysis,beast2}, the Nexus-based language of MrBayes \cite{mrbayes} and the aforementioned Rev programming language used in RevBayes \cite{revbayes}.
...
Unfortunately, BEAST XMLs can be hard to read due to their XML-based, verbose syntax, which is unfamiliar to many users. The deeply nested structures and hierarchical organisation in BEAST XMLs make understanding model components and their relationships more challenging. 
In contrast, more general probabilistic programming languages such as found in JAGS \cite{plummer2003jags}, BUGS \cite{lunn2009bugs, gilks1994language} and Stan \cite{carpenter2017stan} employ more concise, linear structures that simplify comprehension and modification of models. 
Additionally, probabilistic programming languages are more accessible and widely applicable within the statistical modelling community, while BEAST XMLs are more domain-specific. 
...
The syntax for the specification of random variables is similar to probabilistic programming languages such as JAGS \cite{plummer2003jags}, BUGS \cite{lunn2009bugs, gilks1994language} and Stan \cite{carpenter2017stan}.
...
\cite{revbayes,mrbayes,beast,bouckaert2014beastanalysis,beast2}. 
...
In its declarative nature, LPhy's language resembles the XML specification adopted by BEAST 2 \cite{bouckaert2014beastanalysis,beast2}, but shares the central notion of probabilistic graphical models with the Rev language.
...
Future work on integration of LPhy with other popular Bayesian phylogenetic inference tools, such as RevBayes\cite{revbayes}, BEAST \cite{beast}, MrBayes\cite{mrbayes} will increase the flexibility of the framework, and enable easy validation of and comparison between different Bayesian phylogenetic inference engines.
...
\bibitem{bouckaert2014beastanalysis}
Bouckaert R, Heled J, K{\"u}hnert D, Vaughan T, Wu C, Xie D, et~al.
\newblock BEAST 2: A Software Platform for Bayesian Evolutionary Analysis.
\newblock PLoS computational biology. 2014;10(e1003537).
\newblock doi:{10.1371/journal.pcbi.1003537}.
...
\newblock {Accounting for errors in data improves divergence time estimates in
  single-cell cancer evolution}.
...
\bibitem{yule1925ii}
Yule GU.
\newblock A mathematical theory of evolution, based on the conclusions of Dr.
  JC Willis, FRS.
...
S1 Appendix. List of distributions and functions in the LPhy reference implementation.
