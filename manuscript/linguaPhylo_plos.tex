...
family of phylogenetic tools (e.g., \cite{beast,beast2,revbayes}) can
simultaneously attain both the goal of reconstructing phylogenetic
tree(s) (topology and divergence times), as well as that of estimating
macroevolutionary and trait evolution parameters.
...
This flexibility in usage arises organically from Bayesian inference --
the statistical approach all these tools share -- specifically from the
relative ease with which hierarchical Bayesian models can be
built and their parameters sampled with Markov chain Monte Carlo
(MCMC).
* [positive attributes, using all data, uncertainty around estimates]
then difficulty in setting up and reproducing.
