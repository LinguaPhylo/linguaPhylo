...
\subsubsection{Available data types}

nuc, protein, binary, traits ...

\subsubsection{Subst models, ...}


\subsubsection{Data clamping} \textcolor{blue}{TODO: move to Language features}
...
The simulated alignment is encapsulated in a random variable mostly in the sink of the probability graph. However, when the data block has the same random variable containing an existing alignment (e.g. imported from a file), the simulated alignment will be replaced by this imported alignment. This mechanism is called as data clamping.

This is a convenient and useful feature to allow users easily switching the task of a LPhy script between simulating alignments and a real data analysis. 
...
It is important to validate the model before publishing it.

Phylonco model validation example?
