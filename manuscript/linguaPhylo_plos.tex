...
The main features include: simulation from tree generative distributions (e.g., birth-death or coalescent processes), sequence generators (e.g., substitution models \cite{gtr}), and statistical distributions (e.g., Normal distribution). %, tree data and sequence alignment data. 
...
Table \ref{tab:generators} shows a summary of LPhy's API. 
Data can be simulated using LPhy sequence generators, or read in from external data files. 
An input API is provided to parse input sequences from nexus and fasta formats as well as trees from newick format into `LPhy datatypes'. 
...
LPhy probabilistic distributions enable users to generate values from common statistical distributions. 
