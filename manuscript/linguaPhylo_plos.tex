...
\subsubsection{CTMC and Brownian motion}

\textcolor{blue}{TODO: review, and citation multivariate Brownian}

The continuous-time Markov chain (CTMC) \cite{felsenstein1981} generative distribution is one of fundamental sampling distributions in the LPhy language, which the phylogenetic likelihood is derived from. A sequence alignment is simulated for every leaf node, and every direct ancestor node with an id.

There are also alternative sampling distributions developed in LPhy, such as the phylogenetic Brownian motion process\cite{felsenstein1973maximum}, its multivariate version\cite{}, and Ornstein-Ulhenbeck process\cite{felsenstein1973maximum}. Using them, a continuous trait can be simulated from specified models.
For example, running the following LPhy script, $y$ containing 16 continuous values are simulated from a Yule tree $\psi$ with the root value $y0$ sampled from a normal distribution at the diffusion rate $\sigma$ sampled from a log-normal distribution.
...
However, in a real data analysis, it requires to pass the imported alignment rather than the simulated alignment to a Bayesian phylogenetic inference engine. So we implement a mechanism called as data clamping for this kind of tasks. 
The simulated alignment generated by a sampling distribution in the model block is encapsulated in a random variable mostly in the sink of the probability graph. 
