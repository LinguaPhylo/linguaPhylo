...
LPhy provides a framework for phylogenetic model specification, data simulation, and inference using a scripting language called LPhy. 
...
Prior distributions as generative distributions in LPhy include most common statistical distributions, such as the normal distribution, Dirichlet distribution, and log-normal distribution, etc. 
A selected list of distributions available are shown in Table \ref{tab:generators}. 

In the example script below, the `LogNormal()' is a generative distribution sampling values for the random variable $\mu$ with a mean of a constant $-5$ and a standard deviation of $1.25$. 
%and $\theta$. 
...
For example, the variable $\mu$ can be an array instead of a single-value variable, when appending the parameter \textit{replicates = $20$} to the distribution as below. This IID pattern will sample twenty values independently from the same log-normal distribution. 

{\singlespacing
\begin{alltt}
  \textcolor{green}{\(\mu\)} ~ \textcolor{blue}{LogNormal}(\textcolor{gray}{meanlog=}\textcolor{magenta}{-5.0}, \textcolor{gray}{sdlog=}\textcolor{magenta}{1.25}, \textcolor{gray}{replicates=}\textcolor{magenta}{20});
\end{alltt}
}
