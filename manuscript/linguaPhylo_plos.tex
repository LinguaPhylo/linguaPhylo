...
The right-hand side specifies the values of the variable or array. This can be a constant value, array of constant values, deterministic function, stochastic generative distribution, or several nested functions.
...
\noindent A tilde sign $\sim$ is used for specifying stochastic random variables from generators, for example:  
{
  \small
  \begin{listing}
...
\noindent The type of a variable is inferred from the return type of its generator and does not need to be declared. 
...
We have developed LPhyBEAST which provides supports for inference with the BEAST2 engine. 
