...
\section{Methods}
\textcolor{blue}{To Kylie: The LPhy language, as a model specification language, is designed to describe Bayesian phylogenetic models in a representation by the probabilistic graphical model using a concise and readible syntax.}
...

{
  ...
  \}
    \end{alltt}
    \caption{A simple LPhy script for JC + Coalescent. All constants are coloured by magenta, random variables are coloured by green, and generative distributions are coloured by blue. The parameters of generative distributions are coloured by grey. The functions are coloured by dark magenta. The constant variables and deterministic variables are coloured by black.}
    \label{lphy:jccoal}
  \end{listing}
}
\textcolor{blue}{TODO: cannot change caption to Example 1.}

This script concisely specifies a simple phylogenetic analysis or simulation using 5 lines of code inside two blocks, which applies the Jukes-Cantor model and constant-size coalescent tree prior, and assumes the 10 nucletide sequences at the length of 200 evolve at a constant rate (default to 1) through the tree branches.
...
The random variables contain the values generated from generative distributions, and they are also the set of parameters in the specified phylogenetic model to estimate. They are drawn as the green circles. 
But when there is the observed data, such as imported alignment, clamping into a random variable (Section \ref{sec:dataclamping}), the circle representing it will be coloured by blue.

The values of deterministic variables can be generated from either deterministic functions or method calls, but the value will be fixed when given the same inputs to its generator. They are drawn as diamonds. If the value is generated in the data block, then it will be coloured by yellow. Otherwise it will be a red diamond.
...
The code in Example \ref{lphy:jccoal} can be shorten to even 3 lines shown in Example \ref{lphy:jccoal2}, if the constants are not the focus of our study, for instance, making simulations from this model.
...
\textcolor{blue}{TODO: revise?} The generative distributions, functions and methods calls in LPhy are named by well-known terms, as well as their parameters. For example, the blue word ``Coalescent'' is a generative distribution to implement Kingman's coalescent tree prior. 
The variable name in LPhy can be any greek letters, some of which are commonly used for certain parameters in the phylogenetic models. For example, $\Theta$ usually represents the effective population size in a constant-size coalescent model. 

The LPhy script, such as Example \ref{lphy:jccoal}, can be visualised as a probabilistic graphical model, such as Figure \ref{fig:jccoalPGM}. The variables and generators will be one-to-one mapped to the components of the graphical model, vice versa. 
