...
language for model specification and simulation.
A main feature of LinguaPhylo is that it delegates everything unrelated to
the model to other applications, allowing users to focus entirely and
exclusively on defining models.
LPhy makes it easier to report and simulate under complex models, and
to both set up and reproduce inference analyses.
We further introduce two additional independent applications: LPhyStudio
and LPhyBEAST.
LPhyStudio provides a GUI with a command line for an LPhy
interpreter, as well as panels for displaying graphical models and
