...
public class MultispeciesCoalescent implements GenerativeDistribution {
    ...
    public MultispeciesCoalescent(@ParameterInfo(name = thetaParamName, description = "effective population sizes, one for each species (both extant and ancestral).") Value<Double[]> theta,
                                  @ParameterInfo(name = DistributionConstants.nParamName, description = "the number of sampled taxa in the gene tree for each extant species.", optional = true) Value<Integer[]> n,
                                  @ParameterInfo(name = TaxaConditionedTreeGenerator.taxaParamName, description = "the taxa for the gene tree, with species to define the mapping.", optional = true) Value<Taxa> taxa,
    ...
    public void setParam(String paramName, Value value) {
        switch (paramName) {
            ...
            case DistributionConstants.nParamName:
                n = value;
                break;
            case TaxaConditionedTreeGenerator.taxaParamName:
                taxa = value;
                break;
            case SParamName:
                S = value;
