...
public class VariantStyleAlignment extends AbstractAlignment {
    ...
    public VariantStyleAlignment(Alignment root, Map<CellPosition, Integer> variantStore) {
        super(root, variantStore);
        if (root.length() != 1) {
    ...
    private CellPosition getCellPosition(int taxon, int position) {
        String taxaname = getTaxaNames()[taxon];
        CellPosition cellPosition = new CellPosition(taxaname, position);
        return cellPosition;
    }
